...
namespace VSCodexExtension.ViewModels
{
    public sealed class VSCodexToolWindowViewModel : ReactiveObject, IDisposable
    {
        ...
        private string _rateLimitUpdatedAt = "Rate limit telemetry pending";
            ...
            JoinableTaskFactory joinableTaskFactory)
        {
            ...
            ShowSettingsCommand = ReactiveCommand.Create(ShowSettings, outputScheduler: _uiScheduler);
        ...
        private static IReadOnlyList<RateLimitWindowStatus> BuildDefaultRateLimits()
        {
            ...
            var weeklyWindow = new DateTimeOffset(now.Date.AddDays(daysUntilMonday), now.Offset);
            return new[]
            {
                new RateLimitWindowStatus { Label = "Hourly", Remaining = "Waiting for SDK telemetry", UsagePercent = 0, ResetText = "next check " + nextHour.ToString("HH:mm") },
                new RateLimitWindowStatus { Label = "Weekly", Remaining = "Waiting for SDK telemetry", UsagePercent = 0, ResetText = "next check " + weeklyWindow.ToString("d MMM") }
            };
        }
        ...
        private void UpdateRateLimitsFromJson(string json)
        {
            ...
            try
            {
                ...
                var limits = root.SelectToken("rateLimits")
                    ?? root.SelectToken("rate_limits")
                    ?? root.SelectToken("usage.rateLimits")
                    ?? root.SelectToken("usage.rate_limits");
                ...
                var changed = UpdateRateLimitFromToken("Hourly", limits.SelectToken("hourly") ?? limits.SelectToken("hour") ?? limits.SelectToken("requests.hourly") ?? limits.SelectToken("requests.hour"));
                changed |= UpdateRateLimitFromToken("Weekly", limits.SelectToken("weekly") ?? limits.SelectToken("week") ?? limits.SelectToken("requests.weekly") ?? limits.SelectToken("requests.week"));
                if (changed)
                {
                    RateLimitUpdatedAt = "Updated " + DateTimeOffset.Now.ToString("HH:mm");
                }
            }
            catch
            {
        ...
        private bool UpdateRateLimitFromToken(string label, JToken? token)
        {
            ...
            var usagePercent = TokenInt(token, "usagePercent", "usage_percent", "usedPercent", "used_percent");
            var remainingValue = TokenInt(token, "remaining", "available");
            var limitValue = TokenInt(token, "limit", "total", "quota");
            if (!usagePercent.HasValue && remainingValue.HasValue && limitValue.HasValue && limitValue.Value > 0)
            {
                usagePercent = 100 - (int)Math.Round((double)remainingValue.Value / limitValue.Value * 100d);
            }

            if (usagePercent.HasValue)
            {
                status.UsagePercent = usagePercent.Value;
            }

            var reset = TokenString(token, "reset", "resetAt", "reset_at", "resetsAt", "resets_at");
            if (!string.IsNullOrWhiteSpace(reset))
            {
                status.ResetText = "resets " + reset;
            }
