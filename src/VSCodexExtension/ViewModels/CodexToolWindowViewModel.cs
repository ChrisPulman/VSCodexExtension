...
namespace VSCodexExtension.ViewModels
{
    public sealed class CodexToolWindowViewModel : ReactiveObject, IDisposable
    {
        ...
        public CodexToolWindowViewModel(ISettingsStore settingsStore, IMemoryStore memoryStore, ISkillIndexService skillIndex, IMcpConfigService mcpConfig, IWorkspaceContextService workspace, ISessionStore sessionStore, ICodexOrchestrator codex)
        {
            ...
            _subscriptions = new CompositeDisposableLike(_codex.Events.ObserveOnSafe(RxApp.MainThreadScheduler).Subscribe(OnCodexEvent), _skillIndex.Skills.ObserveOnSafe(RxApp.MainThreadScheduler).Subscribe(UpdateSkills), _memoryStore.Memories.ObserveOnSafe(RxApp.MainThreadScheduler).Subscribe(UpdateMemories), _mcpConfig.Servers.ObserveOnSafe(RxApp.MainThreadScheduler).Subscribe(UpdateMcpServers), this.WhenAnyValue(x => x.Prompt).ThrottleDistinct(TimeSpan.FromMilliseconds(180), RxApp.MainThreadScheduler).Subscribe(UpdateFileSuggestions));
            Refresh();
        }
