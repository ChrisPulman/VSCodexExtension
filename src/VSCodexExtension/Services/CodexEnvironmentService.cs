...
namespace VSCodexExtension.Services
{
    ...
    public sealed class CodexEnvironmentService : ICodexEnvironmentService
    {
        ...
        private static async Task<PrerequisiteStatus> CheckCodexSdkAsync(ExtensionSettings settings)
        {
            ...
            {
                script = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Resources", "codex-bridge.mjs");
            }

            if (!File.Exists(script))
            {
                return Missing("Codex SDK", "Required for the primary VSCodex SDK bridge.", "The bundled codex-bridge.mjs file was not found.", "Rebuild or reinstall the VSCodex VSIX.", true);
            }

            var result = await RunProcessAsync(ResolveNodePath(settings.NodePath), Quote(script) + " --check", AppDomain.CurrentDomain.BaseDirectory, 8000).ConfigureAwait(false);
            if (!result.Started)
            {
        ...
        {
            var result = await RunProcessAsync(fileName, arguments, AppDomain.CurrentDomain.BaseDirectory, 5000).ConfigureAwait(false);
            if (!result.Started)
            {
        ...
        {
            return ResolveExecutable(configuredPath, "codex.cmd", Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "npm", "codex.cmd"));
        }
