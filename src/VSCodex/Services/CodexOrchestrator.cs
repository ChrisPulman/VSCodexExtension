...
public sealed class CodexOrchestrator : ICodexOrchestrator
{
    ...
    private async Task<CodexRunResult> RunCliFallbackAsync(CodexRunRequest request, Exception sdkException)
    {
        ...
        {
            throw new InvalidOperationException("VSCodex could not run because the Codex SDK bridge failed and the optional Codex CLI fallback could not complete the request. Open the VSCodex Settings tab, run Check setup, verify the Codex SDK with `npm install -g @openai/codex-sdk`, and only set a Codex profile when it exists in `%USERPROFILE%\\.codex\\config.toml`. Optional CLI fallback install: `npm install -g @openai/codex`. SDK failure: " + sdkException.Message + " CLI failure: " + cliException.Message, cliException);
        }
    }
