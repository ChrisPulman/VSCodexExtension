...
REQUIRED = ['VSCodexExtension.slnx','docs/PLAN.md','src/VSCodexExtension/VSCodexExtension.csproj','src/VSCodexExtension/source.extension.vsixmanifest','src/VSCodexExtension/VSCodexExtensionPackage.cs','src/VSCodexExtension/ToolWindows/CodexToolWindowPane.cs','src/VSCodexExtension/ViewModels/CodexToolWindowViewModel.cs','src/VSCodexExtension/Views/CodexToolWindowControl.xaml','src/VSCodexExtension/Resources/codex-bridge.mjs']
def fail(msg): print('FAIL:', msg); sys.exit(1)
for rel in REQUIRED:
    p = ROOT / rel
    if not p.exists(): fail(f'missing {rel}')
    if p.is_file() and p.stat().st_size == 0: fail(f'empty {rel}')
for rel in ['VSCodexExtension.slnx','src/VSCodexExtension/VSCodexExtension.csproj','src/VSCodexExtension/source.extension.vsixmanifest','src/VSCodexExtension/Views/CodexToolWindowControl.xaml','src/VSCodexExtension/Commands/CodexCommands.vsct']:
